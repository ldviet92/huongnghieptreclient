...
export default function Step3 (props) {
  ...
  const persProspecting = map(pointsProspecting, (item) => {
    return <Personality key={item.Id} uPoint={item} perstypes={perstypes} onChangePerstypeIdChoose={(e) => onChangePerstypeIdChoose(e)} />
  })
  ...
  const perDetails = map(pointsSort, (item) => {
    return <PerstypeDetail key={item.Id} uPoint={item} perstypes={perstypes} onChangePerstypeIdChoose={(e) => onChangePerstypeIdChoose(e)} />
  })

  const onChangePerstypeIdChoose = (perstypeId) => {
    console.log('hghjg', perstypeId)
  ...
  return (
    <div className='mt-5'>
      ...
      <div className='mt-5'>
        <div>
          <h3> <Badge style={{ background: getColor(17), marginTop: 6, marginRight: 5 }} />Rất triển vọng</h3>
          <hr />
          {persProspecting}
        </div>
        <div>
          <h3><Badge style={{ background: getColor(15), marginTop: 6, marginRight: 5 }} />Hứa hẹn</h3>
          <hr />
          {persPromising}
        </div>
        <div>
          <h3><Badge style={{ background: getColor(4), marginTop: 6, marginRight: 5 }} />Thử thách</h3>
          <hr />
          {persChallenging}
        </div>
      </div>
...
function Personality (props) {
  ...
  return (
    <div>
      ...
      {parse(perstype.Jobs)}
      <Button className='mb-4' appearance='link' onClick={() => props.onChangePerstypeIdChoose(perstype.Id)}>
    ...
    <div className='border p-3 mt-4'>
      ...
      <div className='row'>
        ...
        <div className='col-md-3'>
          ...
          <Button className='mb-4 text-wrap' appearance='link' onClick={e => props.onChangePerstypeIdChoose(perstype.Id)}>
            Khám phá thêm các công việc tương tự &gt;
          </Button>
        </div>
      </div>
